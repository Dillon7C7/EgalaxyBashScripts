...
$FFMPEG -i "${INPUTDIR}${INPUT/.mov/}.mp4"  -i $WM_DIR/new_wm_show.png -filter_complex "overlay=x=(main_w-overlay_w-overlay_w/2):y=(main_h-overlay_h-overlay_h/2)" -c:v libx264 -x264opts "crf=8"  -c:a copy -preset veryfast "w${OUTDIR}${INPUT/.mov/}.mp4"
